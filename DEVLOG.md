...
**夸夸弹窗开关**
- 背景设置面板底部新增"夸夸弹窗"开关，采用滑动 Toggle 样式：开启显示绿色，关闭显示灰色，带 200ms 过渡动画
- 开关状态持久化到 `electron-store`（字段 `praiseEnabled`，默认 `true`），重启后保持上次设置
...
**日期前后切换**
- 顶部日期两侧新增 `‹` / `›` 箭头按钮，点击切换前一天 / 后一天
- 切换后自动加载对应日期的 todo 列表，无记录则显示空列表
- 新增 / 完成 / 删除操作均作用于当前选中日期
- 箭头区域加 `no-drag`，不触发窗口拖动

**完成 Todo 夸夸弹窗**
- 勾选某条 todo（未完成 → 已完成）时弹出随机夸夸弹窗，取消完成不触发
- 弹窗结构：顶部随机 emoji（22 个）→ 随机鼓励语（8 条）→ 随机颜文字（16 个）→ 柔和"知道啦"按钮
- 弹窗宽 240px，圆角 20px，白色半透明毛玻璃风格，emoji 作为轻量点缀，文案为视觉重心
